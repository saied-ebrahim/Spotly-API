...
export const checkoutService = async (userID, eventID, quantity, discount = 0) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    cancel_url: `${process.env.BACKEND_URL}/api/v1/checkout/cancel/${order._id}/${event._id}`,
  });
  ...
  try {
    ...
    const emailHTML = `
      ...
      <ul>
        ...
        ${checkout.totalAmount && checkout.currency ? `<li><strong>Total Amount:</strong> ${checkout.totalAmount} ${checkout.currency.toUpperCase()}</li>` : ""}
